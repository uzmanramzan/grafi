...
module.exports.signAccessToken = async (user) => {
  ...
  const token = await JWT.sign(payload, process.env.JWT_SECRETE || 'IAmASecret05dev@@testproject');
  return token;
};
module.exports.verifyJWTToken = async (authToken) => {
  const token = authToken.split(" ")[1];
  const payload = await JWT.verify(token, process.env.JWT_SECRETE || 'IAmASecret05dev@@testproject');
  return payload;
};
module.exports.verificationToken = async (req, res, next) => {
  ...
  try {
    const user = JWT.verify(token, process.env.JWT_SECRETE || 'IAmASecret05dev@@testproject');
    req.user = user;
    next();
  } catch (error) {
